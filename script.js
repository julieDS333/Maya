...
var GREETINGS = [
  ...
  "Dear Maya's cousin, I am Carlos the Oracle.",
  ...
  "Dear mysterious guest, I am Carlos the Oracle.",
...
function resetGame6() {
  g6done = false; currentFriend = null; resetCard('game6');
  document.getElementById('game6-hint').textContent = 'Tap the card to reveal';
  hideEl('game6-speech'); hideEl('game6-type'); hideEl('game6-button'); hideEl('game6-again');
}
function drawGame6() {
  ...
  flipCard(document.getElementById('game6-card'), cardFile);
  setTimeout(function() {
    ...
    showEl('game6-type'); showSpeech('game6', currentFriend.text);
...
var g7done = false;
function resetGame7() {
  g7done = false; resetCard('game7');
  document.getElementById('game7-hint').textContent = 'Tap the card to reveal your mission';
  hideEl('game7-speech'); hideEl('game7-dare'); hideEl('game7-again');
}
function drawGame7() {
  if (g7done) return; g7done = true;
  document.getElementById('game7-hint').textContent = 'Carlos is assigning your fate...';
  var cardFile = pick(MAJOR_ARCANA_FILES);
  flipCard(document.getElementById('game7-card'), cardFile);
  setTimeout(function() {
    var dare = pick(DARES);
    var cardName = cardFile.replace('MW_','').replace('.webp','').replace(/_/g,' ');
    document.getElementById('game7-hint').textContent = cardName;
    document.getElementById('game7-dare-text').textContent = dare.text;
    showEl('game7-dare'); showSpeech('game7', dare.carlos); showEl('game7-again');
  }, 900);
}

var g12done = false;
function resetGame12() {
  g12done = false; resetCard('game12');
  document.getElementById('game12-hint').textContent = 'Tap to reveal what you should have done';
  hideEl('game12-speech'); hideEl('game12-gift'); hideEl('game12-again');
}
function drawGame12() {
  if (g12done) return; g12done = true;
  document.getElementById('game12-hint').textContent = 'Carlos is judging your gift choices...';
  var cardFile = pick(FULL_DECK);
  flipCard(document.getElementById('game12-card'), cardFile);
  setTimeout(function() {
    var gift = pick(GIFTS);
    var cardName = cardFile.replace('MW_','').replace('.webp','').replace(/_/g,' ');
    ...
    showEl('game12-gift'); showSpeech('game12', pick(gift.texts)); showEl('game12-again');
  }, 900);
}
